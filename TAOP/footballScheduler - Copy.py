...
def conflict_checker(horasp: list, horario:dict): 
    ...
    '''
    for item in list(zip(horario.keys(),horario.values())):
        i=1
        ...
        listaOrdenada = sorted(item[1], key = lambda lower:lower.hora_fin[0])
        # print(listaOrdenada)
...
for line in data:
    ...
    if line == "000000 0": 
        for matchday in list(zip(horario.keys(), horario.values())):
            print(matchday[0] + " "+ str(len(matchday[1])))
        ...
        if ( bool(horario) is not False or counter+num is nlines): 
            horas = conflict_checker(horas, horario) #runs for the last added date of matches
        date = spline[0]
        num = spline[1]
        horario[date] = []
        horas[date] = []
